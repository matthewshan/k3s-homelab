...
8. Add a short README or extend `infrastructure/monitoring/helm.md` to capture the manual bootstrap steps for secrets, mirroring the style already used for cert-manager in `docs/setup.md` and the commented guidance in `infrastructure/controllers/cert-manager/external-secret.yaml`.
